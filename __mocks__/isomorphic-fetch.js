...
nodeFetch.mockImplementation((url, options) => {
  ...
  if (url.includes('1rpc.io') || url.includes('infura.io') || url.includes('rpc-mumbai') || 
      url.includes('rpc.syscoin.org') || url.includes('polygon-rpc.com') || url.includes('rpc.tanenbaum.io')) {
    ...
    if (body.method === 'eth_getCode' && body.params && body.params[0]) {
      ...
      const isContract = contractAddresses.some(addr => address.includes(addr));
      return Promise.resolve({
        ok: true,
        status: 200,
        json: () => Promise.resolve({
          jsonrpc: '2.0',
          id: body.id || 1,
          result: isContract ? '0x608060405234801561001057600080fd5b50' : '0x'
        })
      });
    }
        ...
        json: () => Promise.resolve({
          coins: [
            ...
            },
            {
              ...
              symbol: 'SYS',
              market_cap_rank: 100,
              thumb: 'https://assets.coingecko.com/coins/images/119/thumb/syscoin.png',
              large: 'https://assets.coingecko.com/coins/images/119/large/syscoin.png'
            }
          ],
          exchanges: [],
          icos: [],
