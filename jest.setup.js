...
jest.mock('ethers', () => {
  ...
  class MockJsonRpcProvider {
    ...
    async getCode(address) {
      ...
      const isContract = contracts.some(c => normalizedAddress === c.toLowerCase());
      
      // Debug logging
      if (address.toLowerCase().includes('0x0c702f78b889f25e3347fb978345f7ecf4f3861c')) {
        console.log('getCode debug:', { address, normalizedAddress, isContract });
      }
  ...
  return {
    ...
    Contract: jest.fn().mockImplementation((address, abi, provider) => {
      ...
      const isERC721 = erc721Addresses.some(a => a.toLowerCase() === lowerAddress);
      const isERC1155 = erc1155Addresses.some(a => a.toLowerCase() === lowerAddress);
      const isERC20 = erc20Addresses.some(a => a.toLowerCase() === lowerAddress);
      
      return {
        balanceOf: jest.fn().mockImplementation(async (_addr) => {
          // For ERC721 NFT, return 0 for first address, 1 for second
          if (isERC721 && lowerAddress === '0x0c702f78b889f25e3347fb978345f7ecf4f3861c') {
  ...
  const mockStorage = {
    ...
    clear: jest.fn(() => {
      Object.keys(mockStorage).forEach(key => {
        if (key !== 'set' && key !== 'get' && key !== 'remove' && key !== 'clear') {
          delete mockStorage[key];
        }
      });
      return Promise.resolve();
